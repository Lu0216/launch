...
<script>
import { getList } from "@/api/launchReport";
import {
  getMediaList,
  getProjectList
...
export default {
  ...
  methods: {
            ...
            if (!hasProject && !hasMedia && !hasChannel) {
              ...
              this.tableData = this.processData(data.data,params,this.isShow)
                ...
                this.tableData = data.data.map(item => {
                  return {
                    ...item,
                    rate_device_show_cnt: (item.rate_device_show_cnt * 100).toFixed(2) + '%'
                  }
                })
    ...
    updateQidList(data, proj_ids, media_ids) {
      ...
      // 提取 qid 并更新 qidList
      this.qidList = filteredQids.map(item => ({
        id: item.qid,
        // name: item.qid_name // 假设渠道名称字段为 qid_name
      }))
    },
    ...
    getSummaries(param) {
      ...
      // 计算 cpa
      if (!isNaN(sums[6]) && !isNaN(sums[4]) && sums[4] !== 0) {
        sums[8] = (sums[6] / sums[4]).toFixed(2)
      } else {
        sums[8] = 'N/A'
      }

      // 计算 roi
      if (!isNaN(sums[7]) && !isNaN(sums[6]) && sums[6] !== 0) {
        sums[9] = (sums[7] / sums[6]).toFixed(2)
      } else {
        sums[9] = 'N/A'
      }
