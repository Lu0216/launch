    ...
    <el-card shadow="never">
      ...
      <template slot-scope="{ row }">
        ...
        <span v-if="column.prop === 'show_cnt'">
          {{ row.show_cnt == null ? 'null' : row.show_cnt_symbol}}
        </span>
        <span v-if="column.prop === 'arpu'">
          {{ row.arpu == null ? 'null' : row.arpu_symbol + row.arpu }}
        </span>
...
import { getEdit,getAdd ,getList,getBatchEdit} from '@/api/backhaul/quickAppAnchor'
import {
  getMediaList,
  getMediaMasterList,
  getProjectList,
    ...
    return {
      ...
      columns:[
        ...
        },
        {
          ...
          width:'120',
          render: (h, params) => {
            return h('span', params.row.show_cnt == null ? 'null' : params.row.show_cnt_symbol + params.row.show_cnt);
          }
        },
        {
          prop: 'arpu', 
          ...
          width:'120',
          render: (h, params) => {
            return h('span', params.row.arpu == null ? 'null' : params.row.arpu_symbol + params.row.arpu);
          }
        },
        {
          prop: 'click_cnt', 
