<template>
    <div class="offline_wrap">
        ...
        <el-dialog width="400px"  class="deleteDialog" close-on-click-modal :visible.sync="deleteDialogVisible" title="删除">
            <p style="line-height:20px;text-align:left">删除后，{{this.deleteproj}}项目和{{deletemedia}}媒体不再执行实时自动化停投，确认是否删除</p>
...
import { getListRealtime, addRealtime, deleteRealtime } from '@/api/autowhitelist'
...
export default {
    ...
    methods: {
        ...
        getListRealtime() {
            ...
            getListRealtime(datasrc).then((data) => {
        ...
        handleDeleteDialog() {
            deleteRealtime({id:this.deleteId}).then( (data) => {
                if(data.error_code === 0) {
                    this.$message({
                        type: 'success',
