...
import { getEdit,getAdd ,getList,getBatchEdit} from '@/api/learning'
import {
  getMediaList,
  getMediaMasterList,
  getProjectList,
...
export default {
  ...
  methods: {
      ...
      getList(datasrc)
        .then((data) => {
          if (data.error_code === 0) {
            ...
            return { ...item} // 在每个 item 中添加 widgetDisplay 字段
          });
    ...
    oneEditActivate(datasrc) {
      ...
      console.log('qqq',datasrcs)
      getEdit(datasrcs).then((data) => {
        if (data.error_code == 0) {
          this.$message.success("编辑成功");
          this.getList();
        }
